...
5. If fewer than ten qualified topics remain, report fewer. Add the returned evergreen opportunities separately; never label them today's trend.
6. Return the structured list in `templates/topic-list.md`. Do not write the full article until asked, except when the user explicitly asks the Skill to choose and write.

## Output contract

For each candidate include title, intended reader, pain, practical angle, content type, suggested technical depth, score and explanation, trend window, source count, platform count, dated evidence links and risks/availability limits. Mark `trend`, `hybrid` or `evergreen`.
